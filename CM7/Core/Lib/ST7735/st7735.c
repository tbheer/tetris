...
void ST7735_Init()
{		// Vor ausführen idealerweise Reset mit Seesaw ausführen
  //ST7735_Select();
  LCD_CS0
  ;
  //ST7735_Reset();
...
void ST7735_DrawCharS(int16_t x, int16_t y, char c, int16_t textColor, int16_t bgColor, uint8_t size)
{
  uint8_t line;
  int32_t i, j;
  if ((x >= ST7735_WIDTH) || (y >= ST7735_HEIGHT) || ((x + 5 * size - 1) < 0) || ((y + 8 * size - 1) < 0)) return;
...
void ST7735_Drawaxes(uint16_t axisColor, uint16_t bgColor, char *xLabel, char *yLabel1, uint16_t label1Color, char *yLabel2,
...
//					lcd7735_sendData(color);
  ...
  LCD_DC1;  //Set DC HIGH
  error = HAL_SPI_Transmit(&hspi5, buffer, i, 1000);
  if (error == HAL_ERROR)
  {
    error = 0;
  }
...
void lcd7735_senddata(uint8_t data)
{
  ...
  do
  {
    error = HAL_SPI_Transmit(&hspi5, &data, sizeof(data), 0x1);
    i++;
